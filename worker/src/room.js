...
// OpenHill: shared MMO-style slope. Single well-known instance, no player cap,
// no ready/start flow, no host. Players drop in and ski immediately.
//
// Protocol (JSON):
//   client -> DO: { type: 'hello', name?: string }
//   DO -> client: { type: 'welcome', id, color, seed }
//   DO -> peers : { type: 'peerJoined', id, name?, color }
//   client -> DO: { type: 'state', x, y, state, score, speedMult, equipped?, seq }
//   DO -> peers : { type: 'state', id, x, y, state, score, speedMult, equipped? }
//   client -> DO: { type: 'respawn' }
//   DO -> peers : { type: 'respawn', id }
//   DO -> peers : { type: 'peerLeft', id }   (on close)
...
// ---------------------------------------------------------------------------
// OpenHill: persistent shared slope for drop-in/drop-out skiing.
// ---------------------------------------------------------------------------
const HILL_COLOR_PALETTE_SIZE = 10;

export class OpenHill {
  constructor(state, env) {
    this.state = state;
    this.env = env;
    this.seed = null;
  ...
  async fetch(request) {
    ...
    // Lazy-load or create persistent seed
    if (this.seed === null) {
      this.seed = await this.state.storage.get('seed');
      if (this.seed == null) {
    ...
    let color = 0;
    while (color < HILL_COLOR_PALETTE_SIZE && usedColors.has(color)) color++;
    if (color >= HILL_COLOR_PALETTE_SIZE) color = color % HILL_COLOR_PALETTE_SIZE;
    ...
    this.state.acceptWebSocket(server);
    server.serializeAttachment({ id, name: null, color });
  ...
  async webSocketMessage(ws, msg) {
    ...
    switch (data.type) {
      ...
      case 'respawn': {
        this.broadcastExcept(meta.id, { type: 'respawn', id: meta.id });
        break;
      }
      default:
        break;
  ...
  async webSocketClose(ws, code, reason, wasClean) {
    const meta = ws.deserializeAttachment() || {};
    try { ws.close(); } catch {}
    this.broadcastExcept(meta.id, { type: 'peerLeft', id: meta.id });
  }
  ...
  peers() {
    return this.state.getWebSockets().map(ws => {
      const m = ws.deserializeAttachment() || {};
      return { id: m.id, name: m.name, color: m.color, ws };
    });
  }
  broadcastExcept(excludeId, obj) {
    const msg = JSON.stringify(obj);
